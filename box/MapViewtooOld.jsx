...
{ id: 137, lat: 13.7448409, lon: 100.5307858, type: "bulb" },
    ...
    (async () => {
      ...
      (async () => {
        try {
          ...
          // 🟨 ระบายสีตึกสูง (≥15 ม.) เป็นเหลืองอ่อนขอบเข้ม ทับ tile เดิม — ตึกเตี้ยปล่อยให้เป็นเทาอ่อนตามสไตล์ tile อยู่แล้ว ไม่ต้องวาดซ้ำ
          const tallLayer = L.layerGroup().addTo(map);
          for (const b of bl) {
            if (b.h < 15) continue;
            const latlngs = b.ring.map(([lon, lat]) => [lat, lon]);
            L.polygon(latlngs, { color: "#DADCE0", weight: 1, fillColor: "#E8EAED", fillOpacity: 0.78 }).addTo(tallLayer);
          }
        } catch (e) {}
      })();
  ...
  return (
    <div className="bdi-mapwrap" style={{ position: "relative", height: "100%", width: "100%" }}>
      ...
      {/* Skywalk ไม่มีแผงแก้ไข/ปักหมุดแล้ว — ใช้ SVG และไฟ static จากข้อมูลในโค้ดเท่านั้น */}
