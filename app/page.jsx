...
const USER_TABS = [
  { id: "map", label: "แผนที่", icon: "🗺️", code: "UC24" },
...
export default function Page() {
  ...
  const ViewSwitch = () => (
    <div className="bdi-viewswitch" title="สลับมุมมอง มือถือ / คอม">
      {[["mobile", "📱", "มือถือ"], ["auto", "⇄", "อัตโนมัติตามขนาดจอ"], ["desktop", "🖥️", "คอมพิวเตอร์"]].map(([k, ic, label]) => (
        <button key={k} className={view === k ? "on" : ""} title={label} onClick={() => pickView(k)}>{ic}</button>
      ))}
    </div>
  );
  ...
  const MenuBody = () => (
    <>
      ...
      <div style={{ padding: "10px 16px 4px", fontSize: 13, fontWeight: 800, color: "#5F6368", letterSpacing: 1 }}>เมนู</div>
      {menuItems.map((item) => {
        ...
        return (
          <button key={item.code + item.key} onClick={() => pickMenu(item)}
            ...
            <span style={{ flex: 1, minWidth: 0 }}>
              <span style={{ fontSize: 10.5, fontWeight: 800, color: on ? "#1A73E8" : "#5F6368", display: "block" }}>{item.code}</span>
    ...
    <div className="bdi-shell" data-view={dataView}>
      ...
      {desktop ? (
        <aside className="bdi-side">
          <div style={{ padding: "16px 16px 10px", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <b style={{ fontSize: 16, letterSpacing: .6, color: "#202124" }}>KMITL MAP</b>
          </div>
      ...
      <div className="bdi-main">
        <div className="bdi-topbar">
          ...
          <h1 style={{ flex: 1 }}>
            {isUser ? (desktop ? (USER_TABS.find((t) => t.id === tab)?.title || USER_TABS.find((t) => t.id === tab)?.label) : "KMITL MAP") : `${current?.code} · ${ROLE_LABEL[user.role]}`}
          </h1>
          <ViewSwitch />
          <div className="bdi-avatar" title={user.name}>👤</div>
        </div>
        ...
        <nav className="bdi-nav">
          {USER_TABS.map((t) => (
            <button key={t.id} className={tab === t.id ? "on" : ""} onClick={() => setTab(t.id)}>
              <span className="ic" style={{ fontSize: 16 }}>{t.icon}</span>
              {t.label}
            </button>
          ))}
        </nav>
