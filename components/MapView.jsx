...
import { Btn, Card, Field, Input, Select, Textarea, useCollection } from "./ui";
...
export default function MapView({ apiRef, viewMode = "auto", user }) {
  ...
  useEffect(() => {
    ...
    for (const id of Object.keys(kmitlFloorNodes)) {
      ...
      const placeEntry = SC8_SEARCH_NODES.find((s) => s.id === id || s.markerId === id);
      if (placeEntry) {
        marker.bindTooltip(`${placeEntry.icon || t.icon} ${placeEntry.name}`, { permanent: false });
        marker.on("click", () => {
          openPlaceCard(placeEntry.name, [n.lon, n.lat], {
            nodeId: placeEntry.id,
            markerNodeId: placeEntry.markerId || placeEntry.id,
            floor: kmitlFloor,
            icon: placeEntry.icon,
            extract: placeEntry.extract,
          });
        });
      } else if (n.type === "escalator" || n.type === "lift") {
        marker.bindPopup(`${t.icon} ${t.label} #${id}${n.label ? "<br>" + n.label : ""}`);
      } else {
        marker.bindTooltip(`${id}${n.label ? " · " + n.label : ""}`, { permanent: false });
      }
  ...
  // UC25 ใหม่ — แจ้งปัญหา/ขอแก้ไขข้อมูลของสถานที่ที่กำลังเปิดอยู่ใน placeCard
  ...
  return (
    <div className={"bdi-mapwrap " + (viewMode === "desktop" ? "force-desktop" : viewMode === "mobile" ? "force-mobile" : "auto")} style={{ position: "relative", height: "100%", width: "100%" }}>
      ...
      {/* ฟอร์มแจ้งปัญหา/ขอแก้ไขข้อมูลสถานที่ (UC25 ใหม่) */}
